...
C<vee> is a command-line blog tool that is very portable across Unix  sys-
tems.  It provides an interactive as well as a batch interface to main-
tain a log of entries. Formatting is done using a  module  architecture
that allows a high degree of customization. There are minimal flags and
...
Puts vee into batch mode, allowing it to  accept  STDIN  as  the
body  of  the  post.  This  makes vee useful for creating a web-
accessible log index.
...
Specify the directory .vee is in; defaults to the present  work-
ing directory as known to `pwd`

=item -f format

Use  defined format instead of the default, groff ; groff , fold
, and none are supported out of the box.
...
associated with all posts listed with -p

=item -p     Shows what .raw files don't have an entry in $INDEX.

=item -r [1-9*]

Reformats the Nth latests post.

=item -R     Reformats all .raw files; will respect -c
